...
const products = [
  ...
  {"name":"Atta - Wheat Flour - Mehek (5 Kg)","price":"₦10900","image":"https://via.placeholder.com/150","category":"Flours & Atta","offer":"buy 1 get 1"},
...
function scrollToProducts() {
  // Small delay to ensure products are rendered first
  setTimeout(() => {
...
categoryButtons.forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    if (category === "All") {
      ...
      // Filter products that have the "offer" property
...
searchInput.addEventListener("input", () => {
  ...
  // Auto-scroll to products when searching (only if there's text)
  if (query.length > 0) {
    scrollToProducts();
  }
});
